...
class SyncLineParser(object):
	...
	def get_two_major_alleles(self):
		if not self.majminor:
			...
			allele_totals = np.array([sum([y[x] for y in self.seqs]) for x in whichcols]) # get the highest ranked columns in reverse order
			if sorted(allele_totals)[-2] != 0:
                self.majminor =list(allele_totals.argsort()[-1:-3:-1])
            else:  
                self.majminor =list(allele_totals.argsort()[-1])
