...
print.sig.heatmaps <- function(fit,loc,gt,colbr=75,dir=".",p.max = 0.05, min.lfc = 0.0, min.Amean = -10){
  cols1=colorRampPalette(c("green", "white", "red"))(n = colbr)
  cols2=colorRampPalette(c("green", "white", "red"))(n = colbr)
  dgetype="dge.GTs"
  if (gt %in% c("0","2","8")) {
    dgetype="dge.times"
  }
  ...
  for(t.name in dimnames(fit$p.value)$Contrasts ){
    ...
    aheatmap(exprs, color=cols1,  scale="row", distfun="correlation", annCol = GT.times.dges[[dgetype]][[loc]][[gt]]$targets$group,
             filename=paste(bn,loc,gt,t.name,"scaled.pdf",sep="_"), width = 8 , height = 10)
    aheatmap(exprs, color=cols2,  scale="none", distfun="correlation", annCol = GT.times.dges[[dgetype]][[loc]][[gt]]$targets$group,
...
for (loc in locs ) {
  for (gt in names(GT.times.dges[["dge.GTs"]][["DT"]]) ) {
    conts <- makeContrasts(t0t2="time2-time0",t0t8="time8-time0",
                           t2t8="time8-time2",levels=GT.times.dges[["design.GTs"]][[loc]][[gt]])
...
for (loc in locs) {
  ...
  for (gt in names(GT.times.dges[["dge.GTs"]][[loc]])){
    ...
    conts=colnames(fit$contrasts)
    # create directory for saving files
    dir.create("Vennt",showWarnings ='F') 
    # get toptable for each contrast and write to csv
    gt_s=sub("Klotho","K",gt)
    gt_s=sub("VDR","V",gt_s)
    for (cont in conts){
      vTT= topTable(fit,coef=cont,adjust.method="BH",number=nrow(fit$genes),sort.by="none")
      vTT=vTT[,c("gene_symbol","entrezgene","logFC","adj.P.Val")]
      vTT=cbind(rownames(vTT),vTT)
      colnames(vTT)[1]="Feature" 
      vTT=cbind(paste(loc,gt_s,cont,sep=' '),vTT)
      colnames(vTT)[1]="Key"
      write.csv(vTT,file = paste("Vennt/",outspec,"_",cont,".csv",sep=""),row.names = FALSE)
    }
  }
  ...
  for (time in names(GT.times.dges[["dge.times"]][[ loc]]) ) {
    ...
    # get toptable for each contrast and write to file
    tts=list()
    for (cont in conts){
      vTT= topTable(fit,coef=cont,adjust.method="BH",number=nrow(fit$genes),sort.by="none")
      vTT=vTT[,c("gene_symbol","entrezgene","logFC","adj.P.Val")]
      vTT=cbind(rownames(vTT),vTT)
      colnames(vTT)[1]="Feature"
      cont_s=sub("Klotho","K",cont)
      cont_s=sub("FGF23","F",cont_s)
      vTT=cbind(paste(loc,"t:",time,cont_s,sep=' '),vTT)
      colnames(vTT)[1]="Key"
      write.csv(vTT,file = paste("Vennt/",outspec,"_",cont,".csv",sep=""),row.names = FALSE)
    }
  }
...
for (gt in names(GT.times.dges[["dge.GTs"]][["DT"]])){
  for (loc in names(GT.times.dges[["dge.times"]]) ){
    ...
    do_gs_ebrowser(fit,dge,gt,loc,perm=10000)
  }
}
...
#        ea.browse(sbea.gsenr, html.only = TRUE)
  ...
  # write ebrowser files unless MIR
  # in case of KEGG, do different
...
sbea.gsenr <- sbea( method = use.gsenr , eset=eset, gs=kegg.gs , alpha = 0.05, perm = 10000, padj.method = "BH")
...
hh.ora <- sbea( method = "ora", eset=eset, gs=Mm.Migs[["H"]] , alpha = 0.05, perm = 0, padj.method = "BH")
...
# test Neff
neff <- function (cov, chroms) { (cov*chroms -1)/(cov + chroms)}
plot(1:300,neff(1:300,1000), xlab="coverage", type="l", col=c("red"), ylab="Neff",main="effective coverage for 2n chromosomes",ylim=c(0,250))
lines(1:300,neff(1:300,500),col=c("blue"))
lines(1:300,neff(1:300,250),col=c("green"))
lines(1:300,neff(1:300,100),col=c("cyan"))
lines(1:300,neff(1:300,50),col=c("black"))
legend("topleft",c("2n=1000","2n=500","2n=250","2n=100","2n=50"),col=c("red","blue","green","cyan","black"),lty=1)
